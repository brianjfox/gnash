...
// Long-format help bodies (bash 5.3 `help NAME', de-indented; the
// printer re-adds the 4-space indent).  `:' and `shift' keep their
// inline bodies above.  Generated, then hand-checked.
static const struct { const char *name, *body; } kBuiltinLongDoc[] = {
    {"true",
    ...
    {"variables",
     "Common shell variable names and usage.\n\nBASH_VERSION\tVersion information for this Bash.\nCDPATH\tA colon-separated list of directories to search\n\t\tfor directories given as arguments to `cd'.\nGLOBIGNORE\tA colon-separated list of patterns describing filenames to\n\t\tbe ignored by pathname expansion.\nHISTFILE\tThe name of the file where your command history is stored.\nHISTFILESIZE\tThe maximum number of lines this file can contain.\nHISTSIZE\tThe maximum number of history lines that a running\n\t\tshell can access.\nHOME\tThe complete pathname to your login directory.\nHOSTNAME\tThe name of the current host.\nHOSTTYPE\tThe type of CPU this version of Bash is running under.\nIGNOREEOF\tControls the action of the shell on receipt of an EOF\n\t\tcharacter as the sole input.  If set, then the value\n\t\tof it is the number of EOF characters that can be seen\n\t\tin a row on an empty line before the shell will exit\n\t\t(default 10).  When unset, EOF signifies the end of input.\nMACHTYPE\tA string describing the current system Bash is running on.\nMAILCHECK\tHow often, in seconds, Bash checks for new mail.\nMAILPATH\tA colon-separated list of filenames which Bash checks\n\t\tfor new mail.\nOSTYPE\tThe version of Unix this version of Bash is running on.\nPATH\tA colon-separated list of directories to search when\n\t\tlooking for commands.\nPROMPT_COMMAND\tA command to be executed before the printing of each\n\t\tprimary prompt.\nPS1\t\tThe primary prompt string.\nPS2\t\tThe secondary prompt string.\nPWD\t\tThe full pathname of the current directory.\nSHELLOPTS\tA colon-separated list of enabled shell options.\nTERM\tThe name of the current terminal type.\nTIMEFORMAT\tThe output format for timing statistics displayed by the\n\t\t`time' reserved word.\nauto_resume\tNon-null means a command word appearing on a line by\n\t\titself is first looked for in the list of currently\n\t\tstopped jobs.  If found there, that job is foregrounded.\n\t\tA value of `exact' means that the command word must\n\t\texactly match a command in the list of stopped jobs.  A\n\t\tvalue of `substring' means that the command word must\n\t\tmatch a substring of the job.  Any other value means that\n\t\tthe command must be a prefix of a stopped job.\nhistchars\tCharacters controlling history expansion and quick\n\t\tsubstitution.  The first character is the history\n\t\tsubstitution character, usually `!'.  The second is\n\t\tthe `quick substitution' character, usually `^'.  The\n\t\tthird is the `history comment' character, usually `#'.\nHISTIGNORE\tA colon-separated list of patterns used to decide which\n\t\tcommands should be saved on the history list.\nBASHLY_CORRECT\tOpt-in to bash's exact limits, in the spirit of\n\t\t$POSIXLY_CORRECT.  Setting it to `true' pins $GNASH_NAMEREF_MAX to\n\t\tbash's value of 8; any other value, or unsetting it, restores\n\t\tgnash's own defaults.\nGNASH_ENV\tThe name of a file sourced at startup by a non-interactive\n\t\tgnash, the gnash counterpart of $BASH_ENV.\nGNASH_NAMEREF_MAX\tThe maximum number of links a nameref chain may\n\t\tfollow.  Default 100; bash's fixed limit is 8.  A malformed or\n\t\tnon-positive value falls back to the default.\nGNASH_PERSONALITY\tThe shell's active personality: one of bash,\n\t\tzsh, sh, ksh, or csh.\nGNASH_VERSION\tVersion information for gnash itself, independent of\n\t\tthe personality it is emulating.\n"},
    {"while",
     "Execute commands as long as a test succeeds.\n\nExpand and execute COMMANDS-2 as long as the final command in COMMANDS has\nan exit status of zero.\n\nExit Status:\nReturns the status of the last command executed."},
    // gnash-specific: no bash equivalent, so this body is written to match the
    // actual `personality'/`emulate' builtin rather than taken from bash.
    {"personality",
     "Switch the shell's personality at runtime.\n\n"
     "Make gnash behave as another shell, using zsh's `emulate' syntax.  With\n"
     "no PERSONALITY argument, print the name of the current one.\n\n"
     "Personalities:\n"
     "  bash\t\tthe default: GNU Bash 5.3 compatibility\n"
     "  strict-bash\tbash with the $BASHLY_CORRECT behaviour enabled\n"
     "  zsh\t\tZ shell emulation (`emulate' is an alias for this builtin)\n"
     "  sh\t\tPOSIX sh\n"
     "  ksh\t\tKorn shell\n"
     "  csh\t\tC shell\n\n"
     "Options:\n"
...
// The long-format body for NAME, or nullptr when none is recorded.
static const char *long_doc_for(const char *name) {
  for (const auto &d : kBuiltinLongDoc)
    if (std::strcmp(d.name, name) == 0) return d.body;
  return nullptr;
}

// The long documentation body, indented four spaces per line (a blank doc
// line is the indent alone).  An entry's own inline body wins (`:'/`shift'),
// then the long-doc table, and finally the one-line short doc.
static void help_print_body(const BuiltinHelp &h) {
  const char *doc = h.body ? h.body : long_doc_for(h.name);
  if (!doc) doc = h.shortdoc;
  const char *p = doc;
  for (;;) {
    const char *nl = std::strchr(p, '\n');
    size_t len = nl ? static_cast<size_t>(nl - p) : std::strlen(p);
    // bash's help.def prints each doc line as `    <line>' -- but a trailing
    // empty line produced by a doc element's own newline (e.g. the end of the
    // `variables' topic) comes out as a BARE newline, not an indented blank.
    if (len == 0 && !nl) { std::putchar('\n'); break; }
